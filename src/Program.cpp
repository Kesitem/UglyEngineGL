...
ugly::Program::Program(std::shared_ptr<Shader> _vertex_shader, std::shared_ptr<Shader> _fragment_shader)
{
...
ugly::Program::Program(const std::filesystem::path& _vertex_sharder_path,
    const std::filesystem::path& _fragment_shader_path) : Program(
        std::make_shared<Shader>(ShaderType::VERTEX, _vertex_sharder_path), std::make_shared<Shader>(ShaderType::FRAGMENT, _fragment_shader_path))
{
}
...
{
    glUniform1i(getUniformLocation(_name),
        _value);
}
